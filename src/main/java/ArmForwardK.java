...
public class ArmForwardK {
    public static void main(String[] args) throws InterruptedException {
       ...
       Variable[] angles = {new Variable("a1"), new Variable("a2") , new Variable("a3"), new Variable("a4"), new Variable("a5"), new Variable("a6")}; //Radians please
        ...
        for(int i = 0;i<6;i++){
            ...
            link.getMatrix()[0][1] = new Expression(new Negative(new Sin(theta)), new Mul(), new Cos(alpha));
            ...
            link.getMatrix()[1][2] = new Expression(new Negative(new Cos(theta)), new Mul(), new Sin(alpha));
        ...
        Expression costX = new Expression(new Expression(x_coord, new Sub(), new Constant(goalX)), new Pow(), new Constant(2));
        Expression costY = new Expression(new Expression(y_coord, new Sub(), new Constant(goalY)), new Pow(), new Constant(2));

        AlgebraSystem systemCost = new AlgebraSystem(new Term[]{new Expression(costX, new Add(), costY)}, angles);

        SimpleGradientDecent grad = new SimpleGradientDecent(systemCost, 100_000, new double[]{Math.PI/2, 0, 0, 0, 0, 0});

        double[] sol = grad.solve(false);

        System.out.println("Solution: " + Arrays.toString(sol));
        System.out.println("Opt: " + x_coord.getValue() + " " + y_coord.getValue());
